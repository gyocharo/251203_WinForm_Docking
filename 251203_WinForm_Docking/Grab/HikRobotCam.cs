...
namespace _251203_WinForm_Docking.Grab
{
    ...
    internal class HikRobotCam : IDisposable
    {
        ...
        void FrameGrabedEventHandler(object sender, FrameGrabbedEventArgs e)
        {
            ...
            if (_userImageBuffer[BufferIndex].ImageBuffer != null)
            {
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine("Image Convert failed:{0:0x8}", result);
                        return;
                    }
            ...
            try
            {
                ...
                foreach (var devInfo in devInfoList)
                {
                    ...
                    if (devInfo.TLayerType == DeviceTLayerType.MvGigEDevice || devInfo.TLayerType == DeviceTLayerType.MvVirGigEDevice || devInfo.TLayerType == DeviceTLayerType.MvGenTLGigEDevice)
                    {
                        ...
                        uint nIp4 = (gigeDevInfo.CurrentIp & 0xff0000ff);
            ...
            try
            {
                ...
                if (!_device.IsConnected)
                {
                    ...
                    if (_device is IGigEDevice)
                    {
                        ...
                        if (packetSize > 0)
                        {
                            ret = _device.Parameters.SetIntValue("GevSCPPacketSize", packetSize);
                            if (ret != MvError.MV_OK)
                            {
                                Console.WriteLine("Warning : Set Packet Size faile:{0:x8}", ret);
                            }
                            else
                            {
                    ...
                    if (HardwareTrigger)
                    {
                        _device.Parameters.SetEnumValueByString("Trigger Source", "Line0");
                    }
                    else
                    {
                        _device.Parameters.SetEnumValueByString("Trigger Source", "Software");
                    }
        ...
        internal bool GetExposureTime(out long exposure)
        {
            ...
            int result = _device.Parameters.GetFloatValue("Exposure Time", out floatValue);
            if (result == MvError.MV_OK)
            {
